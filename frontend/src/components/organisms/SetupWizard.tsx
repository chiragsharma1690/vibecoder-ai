          ...
          <form onSubmit={handleSetBranch} className="space-y-6 animate-fade-in">
             <div className="space-y-2">
               <label className="flex items-center gap-2 text-sm font-medium text-zinc-300">
                 <GitBranch size={16} className="text-indigo-400" /> Target Branch
               </label>
               
               {/* Note: We could make a custom Select atom later, but a styled native select works perfectly for now */}
               <select 
                 value={selectedBranch} 
                 onChange={(e) => setSelectedBranch(e.target.value)}
                 className="w-full bg-zinc-900/80 border border-zinc-800 rounded-lg px-4 py-3 text-sm text-zinc-100 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/50 transition-all cursor-pointer"
               >
