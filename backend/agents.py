...
def call_llm(prompt: str, format_type=None, temperature=0.1, model="qwen2.5-coder:7b"):
    """Standardized wrapper for Ollama calls."""
    options = {"temperature": temperature, "num_ctx": 8192}
    try:
        response = ollama.generate(model=model, prompt=prompt, format=format_type, options=options)
        raw = response.get("response", "").strip()
        
        # This only catches top-level markdown wraps
        if raw.startswith("```"):
            lines = raw.split("\n")
...
def extract_and_save_files(raw_text: str, repo_path: str, saved_files: list):
    """Parses ---FILE: path--- blocks, cleans rogue markdown, and saves them."""
    if "---FILE:" not in raw_text: return
    ...
    for part in parts[1:]:
        ...
        # 🚨 NEW BUGFIX: Strip rogue markdown wrappers from the content
        if content.startswith("```"):
            # Find the first newline to skip "```javascript" or "```python"
...
def generate_architect_plan(ticket_id: str, jira_context: str, repo_tree: str, feedback: str = None, previous_plan: dict = None):
    ...
    {
        "strategy": "Explanation of solution.", 
        "files_to_modify": ["path.ext"], 
        "new_files": ["path.ext"], 
...
def generate_qa_command(saved_files: list, repo_tree: str, repo_path: str, last_qa_error: str):
    ...
    if not test_cmd:
        ...
        test_cmd = call_llm(fallback_prompt, temperature=0.0)
