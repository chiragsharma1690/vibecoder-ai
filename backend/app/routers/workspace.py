...
@router.post("/connect")
def connect_workspace(request: ConnectRequest, response: Response):
    try:
    ...
    workspace = WorkspaceManager(request.repo_url, request.github_token, session_id=session_id)
    ...
    response.set_cookie(key="vibecoder_session", value=token, httponly=True, samesite="lax")

    return {"status": "success", "workspace_path": repo_path, "branches": workspace.get_available_branches()}

@router.post("/set-branch")
def set_base_branch(request: SetBranchRequest, session: dict = Depends(get_current_session)):
    # Extract the session_id and pass it to the WorkspaceManager
